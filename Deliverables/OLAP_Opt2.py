...
TEMP_TABLES = [
    ...
    """,
    """
    CREATE INDEX idx_genre_location ON temp_genre_location_boxoffice(Genre, Location, TotalBoxOffice);
    """,
    """
    ...
    """,
    """
    CREATE INDEX idx_loc_thet_genre ON temp_location_theater_genre_title(Genre, Location, TheaterName, Title, TotalBoxOffice);
    """,
    """
    ...
    """,
    """
    CREATE INDEX idx_Rrated_title ON temp_r_rated_title_boxoffice(MovieRating, Title, TotalBoxOffice);
    """,
    """
    ...
    """,
    """
    CREATE INDEX idx_r_rated_ny ON temp_r_rated_ny_boxoffice(MovieRating, Title, Location, TotalBoxOFfice);
    """,
    """
    ...
    CREATE TEMPORARY TABLE temp_salary_agent_gender AS (
        ...
        FROM
            factmoviecast_combined fmc
        JOIN
            dimagent dage ON fmc.AgentID = dage.AgentID
        GROUP BY
            dage.AgentName, fmc.Gender
    );
    """,
    """
    ...
    CREATE TEMPORARY TABLE temp_salary_gender_nationality_agent AS (
        ...
        JOIN
            dimagent dage ON fmc.AgentID = dage.AgentID
        GROUP BY
            fmc.Gender, fmc.Nationality, dage.AgentName
    );
    """,
    """
    ...
    CREATE TEMPORARY TABLE temp_salary_american_actors AS (
        ...
        FROM
            factmoviecast_combined fmc
        WHERE
            fmc.Nationality = 'American'
        GROUP BY
            fmc.ActorName, fmc.Nationality
    );
    """,
    """
    ...
    CREATE TEMPORARY TABLE temp_salary_american_actors_agent AS (
        ...
        GROUP BY
            fmc.ActorName, fmc.Nationality, dage.AgentName
    );
    """,
    """
    ...
    CREATE TEMPORARY TABLE temp_rating_reviewer_class_movie_rating_genre AS (
        ...
        JOIN
            factmovie fm ON fmr.MovieID = fm.MovieID
        GROUP BY
            fmr.ReviewerClass, fm.MovieRating, fm.Genre
    );
    """,
    """
