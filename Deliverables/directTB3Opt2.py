...
# Function to fetch and transform data for actor and moviecast
def fetch_and_transform_actor_data_comb(conn):
    cursor = conn.cursor()
    ...
    query = """
            SELECT 
                ...
                ac.Gender 
            FROM actor ac
            LEFT JOIN moviecast mc ON ac.ActorID = mc.ActorID
...
def fetch_and_transform_moviereview_data_comb(conn):
    cursor = conn.cursor()
    
    query = """
            SELECT * FROM 
            moviereviews mr
            LEFT JOIN reviewer rv ON mr.ReviewerID = rv.ReviewerID;
    """
    cursor.execute(query)
    rows = cursor.fetchall()
    ...
    for row in rows:
        ...
        transformed_row = {
            ...
            'ReviewerName': row[4],
            'ReviewerClass': row[5],
            'BirthDate': row[6].strftime('%Y-%m-%d')
        }
        transformed_data.append(transformed_row)
...
def insert_actor_data(conn, data):
    ...
    insert_query = """
        INSERT INTO factmoviecast_combined (MovieID, ActorID, Salary, ActorName, BirthDate, Nationality, AgentID, Gender)
    ...
    try:
        for row in data:
            record = (row['MovieID'], row['ActorID'], row['Salary'], row['ActorName'], row['BirthDate'], row['Nationality'], row['AgentID'], row['Gender'])
            cursor.execute(insert_query, record)
...
def insert_reviewer_data(conn, data):
    ...
    insert_query = """
        INSERT INTO factmoviereview_combined (MovieID, ReviewerID, ReviewRating, ReviewerName, ReviewerClass, BirthDate)
        VALUES (%s, %s, %s, %s, %s, %s)
        ON DUPLICATE KEY UPDATE
    ...
    try:
        for row in data:
            record = (row['MovieID'],row['ReviewerID'], row['ReviewRating'], row['ReviewerName'], row['ReviewerClass'], row['BirthDate'])
            cursor.execute(insert_query, record)
...
def main():
    ...
    try:
        ...
        actor_data = fetch_and_transform_actor_data_comb(conn_source)
        ...
        moviereview_data = fetch_and_transform_moviereview_data_comb(conn_source)
        ...
        insert_reviewer_data(conn_target, moviereview_data)
